...
def run_test_integers():
    ...
    expected = [7, 8, 1, 10, 2, 10, 11]
    answer = integers([(7, 8, 1),
                       ('jake', 'hi', 10),
                       [3.14, 12.6, 2],
                       'hello',
                       [],
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'NWEAERHAJLWOWAOPSNICEMEMED'
    answer = big_letters([(3, 1, 4),  # not a string
                          'Nice WEAthEr we aRe HAving',  # OTSSS
                          (10, 'Ok what is ThiSSS?', 10),  # not a string
                          [],  # not a string
                          ['oops'],  # not a string
                          'Jake Lauteri',  #
        ...
        if type(sequence_of_sequences[k]) is str:
            for j in range(len(sequence_of_sequences[k])):
                if sequence_of_sequences[k][j].isupper():
                    uppers = uppers + sequence_of_sequences[k][j]
    return uppers
# -----------------------------------------------------------------------------
